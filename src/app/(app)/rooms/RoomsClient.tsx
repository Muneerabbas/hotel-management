...
import { Search, BedDouble, Loader2, X, CalendarDays, CheckSquare, Square } from 'lucide-react';
...
const emptyGuest = (): GuestForm => ({ guestName: '', idType: 'aadhaar', idNumber: '', checkIn: today(), checkOut: '' });
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <label className="text-xs text-muted-foreground">
          {ID_TYPES.find(t => t.value === form.idType)?.label} number
        </label>
        <input
...
export function RoomsClient() {
  ...
  const bulkCheckIn = async () => {
    ...
    if (!bulkForm.idNumber.trim())  { toast.error('ID number is required');   return; }
              ...
              {selected.status === 'available' && (
                <>
                  ...
                  <GuestFormFields
                    ...
                    idPlaceholder={ID_PLACEHOLDER[guestForm.idType] ?? 'Enter ID number'}
                  />
                  <button
      ...
      <Panel open={bulkPanelOpen} onClose={closeAll}>
        ...
        <div className="p-5 space-y-4">
          ...
          <GuestFormFields
            ...
            idPlaceholder={ID_PLACEHOLDER[bulkForm.idType] ?? 'Enter ID number'}
          />
          <button
